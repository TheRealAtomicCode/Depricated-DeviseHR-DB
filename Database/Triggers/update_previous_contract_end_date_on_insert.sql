...
-- 	next_year_leave_entitlement INT;
...
-- 		  next_year_leave_entitlement = NEW.contracted_leave_entitlement;
-- 		ELSE
--           IF NEW.end_date <
-- 		  --next_year_leave_entitlement = 0;
-- 		END IF;

--         -- if no previous contract, then new contract will be added by insert
-- 		INSERT INTO leave_year (user_id, company_id, leave_year_start_date, total_leave_entitlement, total_leave_allowance, full_leave_year_entitlement, next_leave_year_entitlement, added_by, is_days)
-- 		VALUES (NEW.user_id, NEW.company_id, saved_contracted_start_date, NEW.contracted_leave_entitlement, NEW.this_contracts_leave_allowence, NEW.contracted_leave_entitlement, next_year_leave_entitlement, NEW.user_id, NEW.is_leave_in_days);
